...
export default async function OrderDetailsPage({ params }: PageProps) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {order.customerNotes ? <section className="rounded-3xl bg-card p-6 ring-1 ring-border"><h2 className="font-serif text-xl">ملاحظات الطلب</h2><p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-muted-foreground">{order.customerNotes}</p></section> : null}
      <section className="grid gap-4 sm:grid-cols-2"><div className="rounded-3xl bg-card p-6 ring-1 ring-border"><MapPin className="size-5 text-accent" /><h2 className="mt-4 font-serif text-xl">عنوان التسليم</h2><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{[order.shippingAddress.fullName, order.shippingAddress.street, order.shippingAddress.city, order.shippingAddress.country, order.shippingAddress.postalCode].filter(Boolean).join('، ')}</p><p className="mt-2 text-sm">{order.shippingAddress.phone}</p></div><div className="rounded-3xl bg-card p-6 ring-1 ring-border"><CreditCard className="size-5 text-accent" /><h2 className="mt-4 font-serif text-xl">الدفع</h2><p className="mt-3 text-sm">{order.paymentMethod}</p><p className="mt-1 text-sm text-muted-foreground">{order.paymentStatus}</p><dl className="mt-4 flex flex-col gap-2 border-t border-border pt-4 text-sm"><div className="flex justify-between gap-4"><dt className="text-muted-foreground">المجموع الفرعي</dt><dd>{new Intl.NumberFormat('en', { style: 'currency', currency: order.currency }).format(order.subtotalMinor / 100)}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted-foreground">الضريبة</dt><dd>{new Intl.NumberFormat('en', { style: 'currency', currency: order.currency }).format(order.taxMinor / 100)}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted-foreground">الشحن</dt><dd>{new Intl.NumberFormat('en', { style: 'currency', currency: order.currency }).format(order.shippingMinor / 100)}</dd></div></dl>{order.estimatedDelivery ? <p className="mt-4 text-xs text-muted-foreground">الوصول المتوقع: {new Date(order.estimatedDelivery).toLocaleDateString('ar')}</p> : null}</div></section>
    </div>
  )
}
