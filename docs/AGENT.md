...
| على الهوست | `/tmp` بس |
| انتهاء الحساب | تلقائي بعد 30 يومًا |
...
📋 قيم متغيرات البيئة (للمستخدم ينفّذها بنفسه):
   ...
   # ملاحظة: `NEXT_PUBLIC_SITE_URL` **مش مستخدم في أي سطر** في الفرونت — متضيفهوش.
   # ودومين المتجر مش متغير منفصل: بيتحسب تلقائيًا من الـ endpoint اللي فوق.
...
# الجزء ج — ربط الفرونت بالمتجر (اتعمل بعد M8)

الجزء دا بيوصف تغييرات **الفرونت (Next.js)** اللي ربطت الموقع بمنتجات
WooCommerce الحقيقية. دا **مكمّل** للجزء ب، مش بديل عنه — وقيود السيرفر
المشتركة في الجزء ب سارية زي ما هي.

## 1) متغيرات البيئة — واحد إلزامي بس

```bash
# .env.local في جذر المشروع
WORDPRESS_GRAPHQL_ENDPOINT=https://<الدومين>/graphql   # إلزامي
AI_GATEWAY_API_KEY=...                                 # للشات بوت + سكربت الترجمة
WORDPRESS_STORE_URL=https://alifleet.com               # اختياري (شوف تحت)
```

- **دومين المتجر مش متغير مستقل.** `wpStoreOrigin()` في `lib/wp/config.ts`
  بيستخرجه من الـ endpoint، فـ `https://cms.alifleet.com/graphql` بيطلع منها
  `https://cms.alifleet.com`.
- استخدم `WORDPRESS_STORE_URL` **بس** لو الـ API على دومين والمتجر على دومين
  تاني (مثال: الـ API على `cms.` والسلة على `alifleet.com`). غير كده سيبه فاضي.
- الموقع **بيشتغل من غير أي متغير**: كل صفحة بترجع 200، وصفحة المنتجات
  بتعرض رسالة "الكتالوج غير متاح مؤقتًا" بدل ما تتكسر (متحقَّق منه بالمتصفح).

## 2) المنتجات كلها لايف من WooCommerce

الداتا الوهمية (12 منتج + `wooId: 1001…1012`) **اتشالت بالكامل**.
`lib/data/parts.ts` بقى أنواع فقط، والداتا بتيجي من `lib/wp/catalog.ts`:

- الاستعلام بيجيب `databaseId` — **رقم المنتج الحقيقي من WooCommerce**، فمفيش
  أرقام مكتوبة بالإيد تبوظ لو حصل reimport. دا كان أخطر بَج في الفرونت.
- السعر بييجي `price(format: RAW)` (رقم عشري خام) مش السعر المنسّق، عشان
  الحساب ميعتمدش على شكل العرض.
- الكاش: 10 دقايق (`CATALOG_REVALIDATE`) عشان 165 منتج ميضربوش السيرفر كل request.
  بيانات الحساب/الجلسة **مش متكاشة** أبدًا.
- التصنيف: بيحاول يطابق تصنيفات WooCommerce على التمانية بتوع الموقع، واللي
  مش متطابق بيروح `other`. الفلاتر الفاضية **مش بتظهر** خالص.

## 3) الترجمة — نصف تلقائي

المنتجات الأصلية عبري فقط. الحل قاموس على القرص + سكربت:

```bash
pnpm translate:dry     # يعرض اللي ناقص من غير ما يكتب حاجة
pnpm translate         # يترجم الناقص بس ويكتبه في القاموس
```

- القاموس: `lib/i18n/product-translations.json`، مفتاحه **النص العبري نفسه**
  بعد تنظيفه. لو النص اتغيّر في WooCommerce، المفتاح ميطابقش والمنتج يرجع
  للعبري — أحسن من إنه يعرض ترجمة قديمة غلط.
- السكربت **بيضيف الناقص بس**، فأي تصحيح يدوي منك بيفضل زي ما هو.
- المنتج اللي لسه متترجمش بيظهر بعنوانه العبري مع `lang="he"` (مهم لقارئ
  الشاشة واتجاه النص)، وفوق القائمة بيظهر تنبيه بعدد المنتجات المتبقية.
- محتاج `AI_GATEWAY_API_KEY` + كارت مسجّل على حساب Vercel، وإلا السكربت
  بيوقف **من غير ما يكتب داتا ناقصة** (متحقَّق منه).

## 4) 🔴 لازم ترفع mu-plugin من جديد — حاجتين اتضافوا

الملف `wordpress/mu-plugin/alifleet-cms.php` **اتعدّل**، وارفعه بنفس طريقة M3:

### أ) `storeSettings` — بيانات التواصل

مفيش طريقة تجيب التليفون/العنوان من GraphQL بغير كده: صفحات options بتاعة
ACF محتاجة **ACF PRO** (عندك المجانية)، وإعدادات WooCommerce نفسها
(`woocommerce_store_address` وغيره) **مش معروضة في WPGraphQL** أصلًا.
فالحقل بيقرأ من `wp_options` مباشرة، والأولوية: ACF لو مليان، وإلا
WooCommerce → Settings. يعني **البيانات اللي مضبوطة في WooCommerce بتظهر
في الموقع تلقائيًا**، ومفيش داتا مكتوبة في الكود.

### ب) `alifleet-cart` — تسليم السلة كاملة

`?add-to-cart=` بتاعة WooCommerce **بتقرا منتج واحد بس**؛ قائمة بفواصل
بتنقل أول سطر وبس (الحل القديم `add-to-cart=1001:1,1002:2` كان غلط).
والحل الرسمي `/checkout-link/` موجود في WooCommerce 10.0+ بس. فالإندبوينت
الجديد بيبني السلة على السيرفر وبيوجّه للـ checkout:

```
/?alifleet-cart=<productId>:<qty>,<productId>:<qty>
```

بيتحقق من كل منتج (موجود / قابل للشراء / متوفر) وبيحدّد الكمية بالمخزون
الفعلي، فمفيش أوردر بيتقبل وبعدين يترفض.

## 5) Cloudflare — لازم تعمله

`alifleet.com` وراه Cloudflare وبيرجّع **403** على طلبات POST للـ `/graphql`
(متحقَّق منه). لو سيبته كده، الـ Next.js مش هيقدر يجيب أي منتج على
الإنتاج. اعمل **WAF exception** لمسار `/graphql`، أو اربط `cms.alifleet.com`
بشهادة حقيقية.

## 6) اللي لسه محتاج قرار منك

- `cms.alifleet.com` **مش في الـ DNS** لسه، وشهادته self-signed. لحد ما
  تربطه استخدم `https://alifleet.com/graphql`.
- بيانات التواصل: املا ACF أو WooCommerce → Settings. أي حقل فاضي **بيختفي
  من الموقع** بدل ما يعرض لينك فاضي.
