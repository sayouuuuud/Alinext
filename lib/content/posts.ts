...
export async function getPosts(): Promise<PostList> {
  ...
  return { posts, featured: posts.find((post) => post.featured) || posts[0] || null, status: posts.length ? 'ok' : 'empty' }
}

export async function getPost(slug: string): Promise<(BlogPost & { content: string }) | null> {
  const content = await getSiteContent()
  const post = content.blog.find((item) => (item.slug || item.id) === slug && item.published !== false)
  if (!post) return null
