...
export async function getAdminSiteContent(): Promise<SiteFullContent> {
  ...
  const admin = createAdminClient()
  // Orders and customers are served by dedicated paginated endpoints
  // (/api/admin/orders, /api/admin/users) and must never ride along with
  // the bulk site-content payload.
  const { data: inquiriesData, error: inquiriesError } = await admin
    .from('inquiries')
    .select('*')
    .order('created_at', { ascending: false })
  if (inquiriesError) throw new Error(`Supabase admin query failed: ${inquiriesError.message}`)

  const inquiries: InquiryItem[] = (inquiriesData || []).map((item) => ({
    id: String(item.id || ''),
