...
const fetcher = async (url: string) => {
  ...
  return response.json() as Promise<{ notifications: AccountNotification[]; unreadCount: number }>
}
...
export function NotificationCenter() {
  ...
  async function markOne(id: string) {
    await fetch('/api/account/notifications', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ notificationId: id }) }).catch(() => {})
    await mutate()
  }
  ...
  if (error) return <div className="rounded-3xl bg-card p-8 text-sm text-muted-foreground ring-1 ring-border">{locale === 'ar' ? 'تعذر تحميل الإشعارات حاليًا.' : 'Could not load notifications right now.'}</div>
