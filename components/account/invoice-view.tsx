...
export function InvoiceView({ order }: { order: TrackingOrder }) {
  ...
  return (
    <article className="mx-auto max-w-4xl bg-card p-6 text-foreground ring-1 ring-border print:max-w-none print:bg-background print:p-0 print:ring-0 md:p-10">
      ...
      <footer className="ms-auto max-w-sm border-t border-border pt-5"><div className="flex flex-col gap-2 text-sm"><div className="flex items-center justify-between gap-8"><span className="text-muted-foreground">{ar ? 'المجموع الفرعي' : 'Subtotal'}</span><span className="font-semibold">{money(order.subtotalMinor, order.currency)}</span></div><div className="flex items-center justify-between gap-8"><span className="text-muted-foreground">{ar ? 'الضريبة' : 'Tax'}</span><span className="font-semibold">{money(order.taxMinor, order.currency)}</span></div><div className="flex items-center justify-between gap-8"><span className="text-muted-foreground">{ar ? 'الشحن' : 'Shipping'}</span><span className="font-semibold">{order.shippingMinor === 0 ? (ar ? 'مجاني' : 'Free') : money(order.shippingMinor, order.currency)}</span></div></div><div className="mt-3 flex items-center justify-between gap-8 border-t border-border pt-3 text-lg font-bold"><span>{ar ? 'الإجمالي' : 'Grand total'}</span><span>{money(order.totalMinor, order.currency)}</span></div><p className="mt-2 text-end text-xs text-muted-foreground">{ar ? `طريقة الدفع: ${order.paymentMethod}` : `Payment method: ${order.paymentMethod}`}</p></footer>
    </article>
  )
}
